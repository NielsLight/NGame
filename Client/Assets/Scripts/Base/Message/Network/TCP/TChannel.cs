...
namespace NGame
{
	public class TChannel : AChannel
	{
		...
		private  void ConnectAsync(IPEndPoint ipEndPoint)
		{
			try
			{
				 this.tcpClient.BeginConnect(ipEndPoint.Address, ipEndPoint.Port,this.BeginConnectCallback,this.tcpClient);
			}
			catch (SocketException e)
			{
				Log.Error(string.Format("connect error: {0}", e.SocketErrorCode));
				this.OnError(e.SocketErrorCode);
			}
			catch (Exception e)
			{
				this.OnError(SocketError.SocketError);
                Log.Error(string.Format("connect error: {0} {1}", ipEndPoint,e));
            }
		}
			...
			try
			{
				...
				while (true)
				{
					...
					if (!stream.CanWrite)
					{
					 ...
					 this.sendBuffer.ReadAsync(stream);
				}
			}
			catch (IOException)
			{
